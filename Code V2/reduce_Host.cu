...
#include "corr2Mex.h"
#include "normXcorr_GPUKernel_V2.cu"
...
int main(int argc,char** argv) {
	...
	M  = AllocateMatrix(5,5, 1);	
	
	// Load Pre and Post to the device
	Matrix *Md;
	cudaMalloc((void **)&Md,sizeof(float)*5*5);
	Matrix Pred = AllocateDeviceMatrix(M);
	...
	reduce<<<dimGrid, dimBlock>>>(M,val,dpX,dpY);
	...
	printf("Y = %d.\n X = %d.\n Correlation is %f",dp_Y,dp_X,corr);
