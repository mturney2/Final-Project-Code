function Dp=GetDp_NormalizedCC_DirectSearch(PreData, PostData, Marker, matchPara, Dp)
...
%             Dp.cc(:,:,i,j)=ttcc;
    ...
    mDD=matchPara.scatterMovingBeamLine+1;
    nDD=matchPara.scatterMovingRangePoint+1;
    regionCC=zeros(nSize, mCC);
        ...
        for j=nBoundaryLine+1:nLine-nBoundaryLine
            if Marker(yStart+matchPara.halfWinLenPoint,j)>0
                ...
                if (yy>=1 & yy<=nSize & xx>=1 & xx<=mCC)
                    ...
                    Dp.y(i,j)=yy+deltaY;
                    Dp.x(i,j)=xx+deltaX;
                    Dp.y(i,j)=Dp.y(i,j)-nDD;
                    Dp.x(i,j)=Dp.x(i,j)-mDD;
                else
                    Dp.y(i,j)=0;
                    Dp.x(i,j)=0;
                end            
