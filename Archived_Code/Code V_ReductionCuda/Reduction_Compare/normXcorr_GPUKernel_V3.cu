...
///////// Maximum Reduction /////////
	...
	int numThreadsX = sX;
	int numThreadsY = sY;
	int bId_temp =  blockIdx.x + blockIdx.y*gridDim.x;
	int xx1 = bx+by*gridDim.x;
	...
	if((tx==0)&&(ty==0)){
		...
		for(int ys=0; ys<numThreadsY; ys++){
			for(int xs=0; xs<numThreadsX; xs++){
				if((xs<sX)&&(ys<sY)){
					tempCorr[bId_temp*(blockDim.x * blockDim.y)+(ys*blockDim.x)+xs]=Corr[(xx1*sY+ys)*sX+xs];
				}
				else{
					tempCorr[bId_temp*(blockDim.x * blockDim.y)+(ys*blockDim.x)+xs]=0.0;
				}
			}
		}
	}
	...
	while(numThreadsY>1){
		...
		if( (ty < numThreadsY) && (tempCorr[bId_temp*(blockDim.x * blockDim.y)+(ty*blockDim.x)+tx] < tempCorr[bId_temp*(blockDim.x * blockDim.y)+((ty+numThreadsY)*blockDim.x)+tx]) ){
		// push the bigger element to the top half of each column
		tempCorr[bId_temp*(blockDim.x * blockDim.y)+(ty*blockDim.x)+tx] = tempCorr[bId_temp*(blockDim.x * blockDim.y)+((ty+numThreadsY)*blockDim.x)+tx];
  	}
	}
	__syncthreads();
	...
	if(ty==0){
		while(numThreadsX>1){
			...
			if((tx<numThreadsX)&&(tempCorr[bId_temp*(blockDim.x * blockDim.y)+tx]<tempCorr[bId_temp*(blockDim.x * blockDim.y)+tx+numThreadsX])){
				tempCorr[bId_temp*(blockDim.x * blockDim.y)+tx] = tempCorr[bId_temp*(blockDim.x * blockDim.y)+tx+numThreadsX];
			}
		}
	}
	__syncthreads();
	...
	if(ty==0 && tx==0){
		quality[bx+by*gridDim.x] = tempCorr[bId_temp*(blockDim.x * blockDim.y)];}
	__syncthreads();
