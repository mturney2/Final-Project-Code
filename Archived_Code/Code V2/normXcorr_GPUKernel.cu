...
__global__ void normXcorr_GPU(Matrix Pre, Matrix Post, float *Corr, params P,float *preMean,float *preVar)
{
	...
	for (int j=0; j<(kerY); j++)
		{
			for (int i=0;i<(kerX);i++){
				if((post_start_row+j)<imgHeight&&(post_start_row+j)>=0&&(post_start_col+i)<imgWidth&&(post_start_col+i)>=0)
					sum2Post += (Post.elements[(post_start_row+j)*(imgWidth)+(post_start_col+i)]-postMean[tx+ty*sX])*(Post.elements[(post_start_row+j)*(imgWidth)+(post_start_col+i)]-postMean[tx+ty*sX]);
				else
					sum2Post += (postMean[tx+ty*sX])*(postMean[tx+ty*sX]);
			}
		}
...
/////Cross-correlation Calculation/////
		...
		for(int i=0;i<kerX;i++){	
			if((post_start_row+j)<imgHeight&&(post_start_row+j)>=0&&(post_start_col+i)<imgWidth&&(post_start_col+i)>=0&&(start_row+j)<imgHeight&&(start_row+j)>=0&&(start_col+i)<imgWidth&&(start_col+i)>=0)			
				CC += (Pre.elements[(i+start_col)+(j+start_row)*imgWidth]-preMean[bx+by*gridDim.x])*(Post.elements[(post_start_row+j)*(imgWidth)+(post_start_col+i)]-postMean[tx+ty*sX]);
					...
					CC += (0.00 - preMean[bx+by*gridDim.x])*(Post.elements[(post_start_row+j)*(imgWidth)+(post_start_col+i)]-postMean[tx+ty*sX]);
			else
					CC += (0.00 - preMean[bx+by*gridDim.x])*(0.00 - postMean[tx+ty*sX]);
	...
	Corr[tx+sX*(ty+sY*xx)] = CC/sqrt(preVar[bx+by*gridDim.x]*postVar[ty*sX+tx]);
