...
%         if sum(sum(marker(yStart:yEnd, xStart:xEnd)))>0
        if marker(currentPoint,j)>0
            ...
            tt1=tt1-mean(tt1(:)); % for Bmode only, because of the amplitude
...
Dp.y(:, nBoundaryLine+1:nLine-nBoundaryLine)=Dp.y(:, nBoundaryLine+1:nLine-nBoundaryLine)-matchPara.scatterMovingRangePoint-1;
Dp.x(:, nBoundaryLine+1:nLine-nBoundaryLine)=Dp.x(:, nBoundaryLine+1:nLine-nBoundaryLine)-matchPara.scatterMovingBeamLine-1; 
