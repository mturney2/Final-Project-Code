...
//#include "corr2Mex_V5.h" // Change also in header
...
void CorrelationOnDevice(const Matrix M, const Matrix N, float *CorrH, params parameters,float *quality,int *dpX, int *dpY);
...
int main(int argc,char** argv) {
	...
	int *dpX;
	dpX = (int*)malloc(DisplacementSize*sizeof(int));
	int *dpY;
	dpY = (int*)malloc(DisplacementSize*sizeof(int));
	...
	CorrelationOnDevice(Pre, Post, CorrH, parameters,quality,dpX,dpY); // Execution Model for GPU is set up in this function
...
void CorrelationOnDevice(const Matrix Pre, const Matrix Post, float *CorrH, params parameters,float *quality,int *dpX, int *dpY)
{
	...
	cudaMemcpy(dpX,dpX_D,sizeof(int)*parameters.numX*parameters.numY,cudaMemcpyDeviceToHost);
	cudaMemcpy(dpY,dpY_D,sizeof(int)*parameters.numX*parameters.numY,cudaMemcpyDeviceToHost);
...
Matrix AllocateMatrix_Pre(int height, int width,int init) // 1 is file read/ 0 is just allocation
{
	...
	M.elements = NULL;
	FILE *fp;
	fp = fopen("Pre_Padded_equal.inp","r");
